...
if (isset($_SESSION['user_id'])) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (isset($_POST['remove_photo'])) {
            $update_sql = "UPDATE users SET profile_photo = NULL WHERE id = $user_id";
            if ($conn->query($update_sql) === TRUE) {
                $_SESSION['success'] = "Foto profil berhasil dihapus.";
            } else {
                $_SESSION['error'] = "Gagal menghapus foto profil: " . $conn->error;
            }
...
<div class="container">
    ...
    <h1 class="profile-title">Edit Profile</h1><br>

    <?php if ($error): ?>
        <p style="color: red; border: 2px solid red; padding: 5px;"><?= $error ?></p>
    <?php endif; ?>
    <?php if ($success): ?>
        <p style="color: var(--color-secondary); border: 2px solid var(--color-accent); padding: 5px;"><?= $success ?></p>
    <?php endif; ?>
    <br>
    ...
    <form action="profile.php" method="POST" class="pixel-form" enctype="multipart/form-data" style="display: flex; gap: 20px;">
        ...
        <div style="flex: 1; text-align: center;">
            ...
            <button type="submit" name="remove_photo" class="pixel-button" style="padding: 5px 15px; cursor: pointer; " onclick="return confirm('Are you sure you want to remove your profile photo?')">Remove</button>
        </div>
