        ...
        <div style="flex: 1; border: 4px solid var(--color-text); padding: 20px; background-color: var(--color-secondary);">
            ...
            <?php if ($active_build): ?>
                <div style="text-align: center; margin-bottom: 15px;">
                    <img id="active_avatar_img" src="assets/images/avatar_<?= $active_build['avatar_id'] ?? 'default' ?>.png" alt="Active Avatar" style="width: 150px; height: auto; border: 2px solid var(--color-text);">
                </div>

                <h4>Base Class: <?= $active_build['avatar_name'] ?></h4>
                <h4>Weapon:</h4>
                <p>— <?= $active_build['weapon_name'] ?> (ATK: <?= $active_build['attack_stat'] ?>)</p>
                
                <h4>Skills:</h4>
                <p>— <?= $active_build['skill_names'] ?? 'None' ?></p>
                
                <form action="backend/build.php" method="POST">
                    <input type="hidden" name="active_build_id" value="<?= $active_build['build_id'] ?>">
                    <button type="submit" name="confirm_active_build" class="pixel-button" style="width: 100%; margin-top: 15px;" disabled>Confirmed</button>
                </form>
            <?php else: ?>
